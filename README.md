...
Текущий release `1.2.0` поддерживает:
...
Session checkpoint использует штатный append-only `SessionHistoryStorage`: после 30 секунд dirty-state или 256 обновлений pending batch сбрасывается на следующей безопасной границе `eventId` либо agent checkpoint. Один потоковый block не разрезается на повторяющиеся IDs. Таймер в idle не работает, вся `.events` история остаётся на диске, а файлы не копируются и не переписываются целиком.
...
- Checkpoint ограничивает только незаписанный backend-буфер. Он не обрезает старые события и не подменяет отдельную frontend-пагинацию очень больших открытых чатов.
