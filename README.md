...
- Последнее валидное состояние лимитов не исчезает при временном сбое refresh.
- Активная история сессии дописывается на диск по ходу работы ограниченными checkpoint batches.
- Большие чаты открываются через проверяемый компактный UI-кэш без изменения полной истории.
...
Текущий release `1.3.0` поддерживает:
...
Индикатор не читает диагностическую SQLite. Version-checked bridge внутри установленного ACP опрашивает официальный `account/rateLimits/read` раз в 20 секунд и ротирует два поколения небольшого JSON snapshot в активном `CODEX_HOME`. Пустой ответ не заменяет last-known-good данные, а частичный ответ сохраняет ранее известные buckets. Окна строятся по `windowDurationMins`: если backend вернул только недельное окно, UI не дорисовывает несуществующий 5-часовой лимит. Именованные buckets сопоставляются с выбранной моделью по `limitName`; остальные модели используют объявленный backend default bucket.

Session checkpoint использует штатный append-only `SessionHistoryStorage`: после 30 секунд dirty-state или 256 обновлений pending batch сбрасывается только на следующей безопасной границе `eventId`. Один потоковый block не разрезается на повторяющиеся IDs. Таймер в idle не работает, вся `.events` история остаётся на диске, а исходные файлы не копируются и не переписываются.

Для `.events` больше 8 МБ frontend читает соседний `.ui-cache-v2`. Кэш сохраняет все пользовательские prompts и assistant Markdown, последние 600 событий и ограниченные представления тяжёлых terminal/diff payload; старые технические tool events остаются в полном append-only журнале. Кэш строится атомарно, инвалидируется по размеру и времени изменения source, проходит штатную typed-десериализацию и при любой ошибке уступает место оригинальному reader. Установщик заранее готовит кэши при закрытой IDE.
...
- обновляет предыдущую версию этого патча через проверенный чистый бэкап;
- готовит и валидирует UI-кэши больших чатов;
...
- Патч не меняет лицензирование JetBrains, netfilter, MCP-конфигурацию, model selector, resume/reconnect или compaction сессий.
- Checkpoint ограничивает только незаписанный backend-буфер. UI-кэш является удаляемым производным представлением и никогда не используется для rollback или восстановления серверной сессии.
